...
//          if ((nat[j] = (spaces[j]->bc_type_callback(marker) == BC_NATURAL)))
        ...
        if(bnd[edge] == 1)
        {
					...
					for (unsigned int ww = 0; ww < s->bfsurf.size(); ww++)
					{
						...
						if ((bfs->area != ANY_BOUNDARY_EDGE && bfs->area != ANY_EDGE) && !wf->is_in_area(marker, bfs->area)) continue;
					...
					for (unsigned int ww = 0; ww < s->lfsurf.size(); ww++)
					{
						...
						if ((lfs->area != ANY_BOUNDARY_EDGE && lfs->area != ANY_EDGE) && !wf->is_in_area(marker, lfs->area)) continue;
        ...
        else
        {
					...
					for (unsigned int ww = 0; ww < s->bfsurf.size(); ww++)
					{
						...
						if ((bfs->area != ANY_EDGE && bfs->area != ANY_INNER_EDGE) && !wf->is_in_area(marker, bfs->area)) continue;
					...
					for (unsigned int ww = 0; ww < s->lfsurf.size(); ww++)
					{
						...
						if ((lfs->area != ANY_EDGE && lfs->area != ANY_INNER_EDGE) && !wf->is_in_area(marker, lfs->area)) continue;
						m = lfs->i;  fv = spss[m];  am = &al[m];
