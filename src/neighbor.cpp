...
void NeighborSearch::set_active_edge(int edge)
{
	...
	// The size of the neighboring element is either smaller or greater than that of the central element.
	else
	{
...
void NeighborSearch::finding_act_elem_up( Element* elem, int* orig_vertex_id, Node** road_vertices, int n_road_vertices)
{
	...
	// Find out if between the vertices p1 and p2 there is an active edge.
	...
	if(vertex != NULL)
	{
		if (n_road_vertices == 0)
			...
			if(n_road_vertices == max_n_trans - 1)
				exit(1); // tohle tady nema proste vubec osetreny nikde
			else
	...
	// We found an active edge between the current p1 and p2 points.
	else
	{
		for (int i = 0; i < 2; i++)
		{
			// This condition tests on which side there is the neighboring active element.
			if ((edge->elem[i] != NULL) && (edge->elem[i]->active == 1))
			{
				...
				// Edge info and its push into the vector.
					...
					//nicmene se to muze udelat uz ted.
				NeighborEdgeInfo local_edge_info;
				local_edge_info.local_num_of_edge = neighbor_edge;
				...
				for(int j = n_road_vertices; j > 0; j-- )
				{
					...
					{
						Node* n = mesh->peek_vertex_node(road_vertices[j]->id, p1);
						// The vertex at half point is between road_vertices[j] and p2
						if(n == NULL)
						{
						...
						else
						{
							...
							// The vertex at half point lies between p1 and road_vertices[j]
							else
							{
				...
				// Push back to respective vectors.
				n_neighbors++;
				neighbors_id.push_back(neighb_el->id);
				neighbors.push_back(neighb_el);
			}
