      ...
      for (unsigned int edge = 0; edge < e0->nvert; edge++)
      {
        ...
        if(bnd[edge] == 1)  // Assemble boundary edges:
        {
          ...
          for (unsigned int ww = 0; ww < s->mfsurf.size(); ww++)
          {
            ...
            // (as usual in H2D for standard FEM). If the user added the form in the area corresponding to the edge marker,
            // or just to any H2D_DG_BOUNDARY_EDGE, it will be evaluated since non-natural boundary conditions may be
            // enforced weakly in DG.
            if ((!nat[m] || !nat[n]) && mfs->area != H2D_DG_BOUNDARY_EDGE && !wf->is_in_area(marker, mfs->area)) continue;
          ...
          for (unsigned int ww = 0; ww < s->vfsurf.size(); ww++)
          {
            ...
            if (!nat[m] && vfs->area != H2D_DG_BOUNDARY_EDGE && !wf->is_in_area(marker, vfs->area)) continue;
